...
#define SMSetError(Error, Code, Message, ...) \
	do { \
		if ((Error) && *(Error) == nil) {\
			...
			*(Error) = [NSError errorWithDomain:@"SMJEvaluatorFactoryErrorDomain" code:(Code) userInfo:@{ NSLocalizedDescriptionKey : ___message }]; \
		} \
	} while (0) \

#define SMBoolToEvaluateResult(BoolValue) ({	\
	SMJEvaluatorEvaluate __value;				\
												\
	if (BoolValue)								\
		__value = SMJEvaluatorEvaluateTrue;		\
	else										\
		__value = SMJEvaluatorEvaluateFalse;	\
												\
	__value;									\
})
...
+ (NSDictionary <NSString *, id <SMJEvaluator>> *)buildEvaluatorsMap
{
	...
	// Predicate Match.
	/*
	map[SMJRelationalOperatorMATCHES] = [SMJEvaluatorGeneric evaluatorWithBlock:^SMJEvaluatorEvaluate(SMJValueNode *leftNode, SMJValueNode *rightNode, id<SMJPredicateContext> context, NSError **error) {
		return SMJEvaluatorEvaluateError;
	}];
	*/
